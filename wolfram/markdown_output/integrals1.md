...
is true for the product of a step function with a scalar. So the step functions on an interval form a vector space $\mathcal{S}[a, b]$.
...
$$
\sup _{\phi \le f}\int_a^b \phi(x) \, dx = \inf _{\psi \ge f}\int_a^b \phi(x) \, dx
$$

and we can define:

$$
\int_a^b f(x) \, dx := \sup _{\phi \le f} \int _a^b\ phi(x) \, dx  = \inf _{\psi \ge f}\int_a^b \phi (x) \, dx
$$
...
intermediate sums. This needs some terminology: we call $\left\{x_1, x_2, \dots , x_n\right\}$ 
...
Here is the definition: A function $f$ is R-integrable on $[a,b]$ iff, for any $\epsilon > 0$, 
...
$$
lim_{n\to\infty }\int_a^b f_n(x) \, dx = \int _a^blim_{n\to\infty }f_n(x)
$$
...
$m \phi(x) \le f(x) \phi(x) \le M \phi(x)$ for $x \in [a,b]$

$m \int_a^b \phi(x) \, dx \le \int_a^b f(x) \phi(x) \, dx \le  M \int_a^b \phi(x) \, dx$

$\int _a^b f(x) \phi(x) \, dx = \mu \int_a^b \phi(x) \, dx$

$ \exists \mu \in [a,b] : f(\xi) = \mu$
...
\text{                                }=(u-x)f'(x)+\int _x^u(u-t)f'[t]dt\text{=.}\dots
...
$$
\quad f=O[g]\text{        }\Longleftrightarrow \text{    }\frac{f(x)}{g(x)}\le C\quad f=O[1]\text{        }\Longleftrightarrow \text{ 
  }f(x)\le C\quad f=O\left[x^n\right]\text{      }\Longleftrightarrow \text{    }f(x)\leC x^n
$$
